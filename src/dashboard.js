...
const Dashboard = () => {
  ...
  const updateAgentLocation = async (lat, lng) => {
    try {
      ...
      await axios.put(`https://recycle-backend-lflh.onrender.com/api/agents/${agentId}/location`, {
        latitudes: lat,
        longitudes: lng,
      });
  ...
  const handleUpdateSettings = async () => {
    try {
      ...
      const response = await axios.put(`https://recycle-backend-lflh.onrender.com/api/agents/${agentId}/settings`, {
        agentStatus: status,
        type: deliveryType,
      });
