...
const LoginPage = ({ onLogin }) => {
  ...
  const handleLogin = async () => {
    try {
      const response = await axios.post('https://recycle-backend-lflh.onrender.com/api/agent_login', { username, password });
      const { token, id } = response.data; // Assuming the backend returns the agentId
      console.log('data..', response.data);
      localStorage.setItem('token', token);

      // Fetch agent info
      const agentResponse = await axios.get(`https://recycle-backend-lflh.onrender.com/api/agents/${id}`, {
        headers: {
  ...
  const handleSignup = async () => {
    try {
      ...
      const response = await axios.post('https://recycle-backend-lflh.onrender.com/api/agent_signup', {
        id: agentId,
        name,
        age,
        contactNumber,
